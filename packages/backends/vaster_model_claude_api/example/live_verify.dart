...
Future<void> main() async {
  ...
  final turn1 = await model.generate(
    ModelRequest(
      systemInstruction: ChatMessage.system('Use the read_file tool to answer questions about files.'),
  ...
  if (call == null) {
    ...
    final turn2 = await model.generate(
      ModelRequest(
        systemInstruction: ChatMessage.system('Use the read_file tool to answer questions about files.'),
        messages: [
          ChatMessage.user('What does /mem/greeting.txt contain?'),
          ChatMessage(role: Role.model, parts: [call]),
          ChatMessage.toolResponse(call.callId, call.name, {'content': 'hello from the Vaster VFS'}),
        ],
        tools: const [tool],
        generationConfig: const GenerationConfig(maxOutputTokens: 2000),
      ),
  ...
  final r3 = await model.generate(
    ModelRequest(
      messages: [ChatMessage.user('Extract: "Deploy service alpha to region eu-west-1 at priority 3"')],
