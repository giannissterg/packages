...
void main() {
  ...
  test('client ↔ sidecar over rings, with zero-copy KV frame reuse', () async {
    ...
    final kv = LlamaFfiKvCacheController(worker: worker, namePrefix: 'vsc_kv_${stamp % 100000}_');
    ...
    final client = MmapVasterModel(ring: req, responseRing: res, frameResolver: kv);
    ...
    expect(cold.usage.source, UsageSource.measured, reason: 'usage crossed the wire intact');
    ...
    final handle = await kv.materialize(contentFingerprint: fingerprint, content: knowledge);
    final warm = await client.generate(
      request(
        hints: [const ContextCacheHint(regionId: 'facts', contentFingerprint: fingerprint)],
      ),
    );
    expect(
    ...
    expect(warm.text, cold.text, reason: 'zero-copy reuse must not change the completion');
  }, skip: skip);

  test('sidecar answers unknown actions and failures as typed errors', () async {
    final stamp = DateTime.now().microsecondsSinceEpoch;
