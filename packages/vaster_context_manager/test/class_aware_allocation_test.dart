...
void main() {
  ...
  const budget = TokenBudget(
      maxContextTokens: 10000, reservedOutputTokens: 0, reservedToolTokens: 0);
  ...
  group('ClassAwareAllocationStrategy', () {
    test('reservations are satisfied before weighted surplus', () {
      final table = ContextClassTable.standard.withOverrides([
        ...
        const ContextClass(
            name: 'history',
            band: 30,
            share: BudgetShare(weight: 100)), // greedy competitor
      ]);

      final compiled = strategyWith(table).allocate(regions: [
        for (var i = 0; i < 8; i++)
          region('hist_$i', classId: 'history', tokens: 2000, order: i),
    ...
    test('caps bound a class even with surplus available', () {
      final table = ContextClassTable.standard.withOverrides([
        const ContextClass(
            name: 'scratch',
            band: 90,
            share: BudgetShare(maxTokens: 1000)),
      ]);

      final compiled = strategyWith(table).allocate(regions: [
        for (var i = 0; i < 5; i++)
          region('s_$i', classId: 'scratch', tokens: 400),
      ], budget: budget);

      expect(compiled.classUsage['scratch']!.admittedTokens,
          lessThanOrEqualTo(1000));
      expect(compiled.evictedRegions, isNotEmpty);
    });

    test(
        'PREFIX STABILITY: growing volatile pressure never changes a '
      ...
      List<String> knowledgeLayout(int historyRegions) {
        final compiled = strategy.allocate(regions: [
          ...knowledge,
          for (var i = 0; i < historyRegions; i++)
            region('hist_$i', classId: 'history', tokens: 1500, order: i),
        ], budget: budget);
        return compiled.includedRegions
            .where((r) => r.classId == 'knowledge')
            .map((r) => r.id)
            .toList();
      }
      ...
      expect(pressured, equals(calm),
          reason: 'the stable band must be a function of its own content only');
    });
    ...
    test('cache-stable classes shed from the tail only (prefix cut)', () {
      ...
      final ids = compiled.includedRegions
          .where((r) => r.classId == 'knowledge')
          .map((r) => r.id);
      expect(ids, equals(['know_a']),
          reason: 'tail cut at know_b; know_c must not be reordered in');
    });
    ...
    test('repeat compiles are byte-identical (deterministic layout)', () {
      ...
      final second = strategy.allocate(
          regions: regions.reversed.toList(), budget: budget);

      expect(first.includedRegions.map((r) => r.id).toList(),
    ...
    test('dropOldest sheds the oldest members under pressure', () {
      ...
      final compiled = strategyWith(table).allocate(regions: [
        for (var i = 0; i < 5; i++)
          region('hist_$i', classId: 'history', tokens: 1000, order: i),
      ], budget: budget);

      expect(
          compiled.includedRegions.map((r) => r.id), equals(['hist_2', 'hist_3', 'hist_4']),
          reason: 'newest survive; layout stays chronological');
      expect(compiled.evictedRegions.map((r) => r.id),
          containsAll(['hist_0', 'hist_1']));
    });
    ...
    test('multiple system regions concatenate in layout order', () {
      final compiled = strategyWith(ContextClassTable.standard).allocate(
        regions: [
          region('sys_b',
              classId: 'system', role: Role.system, text: 'Second rule.', order: 1),
          region('sys_a',
              classId: 'system', role: Role.system, text: 'First rule.', order: 0),
          region('chat', classId: 'history', text: 'hello'),
        ],
        budget: budget,
      );

      expect((compiled.systemInstruction!.parts.first as TextPart).text,
          equals('First rule.\n\nSecond rule.'));
      expect(compiled.messages, hasLength(1),
          reason: 'system content never leaks into messages');
    });

    test('unevictable overflow is a hard error naming the classes', () {
      expect(
        () => strategyWith(ContextClassTable.standard).allocate(regions: [
          region('sys', classId: 'system', role: Role.system, tokens: 20000),
        ], budget: budget),
        throwsA(isA<ContextOverflowError>()
            .having((e) => e.offendingClasses, 'classes', contains('system'))),
      );
    });
