...
final class ReliabilityBenchmarks {
  ...
  static File _fixture(String name) => [
        '../vaster_playground/test/fixtures/$name',
        'packages/host/vaster_playground/test/fixtures/$name',
      ].map(File.new).firstWhere((f) => f.existsSync(),
          orElse: () => throw StateError('fixture not found: $name'));

  static ReplayEnvelope _envelope(String name) => const ReplayEnvelopeCodec()
      .decodeString(_fixture(name).readAsStringSync());
  ...
  static ReliabilityBenchmark _sddMultiAgent() => ReliabilityBenchmark(
        id: 'sdd_multi_agent',
        exercises: 'multi-agent Tasks (transactional by default), '
        ...
        program: () => VasterProgram.fromJson(
            _envelope('sdd_fidelity.replay.json').programJson!),
        ciVm: () async => VasterVMEngine.bootstrap(
            config: VMConfig(
                defaultModel: ReplayVasterModel(
                    tape: _envelope('sdd_fidelity.replay.json').tape))),
      );
  ...
  static ReliabilityBenchmark _storyLinesLlama() => ReliabilityBenchmark(
        ...
        exercises: 'prompt chaining, local-inference fidelity '
            '(second real backend)',
        backendLabel: 'tape:llama-ffi',
        scorer: const HaltedScorer(),
        ciTrials: 1,
        program: () => VasterProgram.fromJson(
            _envelope('story_lines_v2.replay.json').programJson!),
        ciVm: () async => VasterVMEngine.bootstrap(
            config: VMConfig(
                defaultModel: ReplayVasterModel(
                    tape: _envelope('story_lines_v2.replay.json').tape))),
      );

  // ── Fault benchmarks: deterministic injection, CI-only ─────────────────

  static VasterProgram _compile(List<VasterNode> children,
          {required String result}) =>
      const BasicWorkflowCompiler().compile(Pipeline(
        name: 'benchmark',
  ...
  static ReliabilityBenchmark _retryHeals() => ReliabilityBenchmark(
        id: 'retry_heals',
        exercises: 'Resilient retry loop (REL-P2): transient failures '
            'heal within the declared ceiling',
        backendLabel: 'fault:500-500-ok',
        scorer:
            const AllOfScorer([HaltedScorer(), ContainsScorer('recovered')]),
        program: () => _compile([
          const Resilient(
            attempts: 3,
            child: Prompt(Template.text('fetch the status'),
                output: Binding('status')),
          ),
        ], result: 'status'),
        ciVm: () async {
          var calls = 0;
          return VasterVMEngine.bootstrap(
              config: VMConfig(
                  defaultModel: FakeVasterModel(handler: (request) {
            calls++;
            if (calls <= 2) throw StateError('API error 500 outage');
            return ModelResponse(
                message: ChatMessage.model('the service recovered'));
          })));
        },
      );
  ...
  static ReliabilityBenchmark _fallbackServes() => ReliabilityBenchmark(
        ...
        ciVm: () async {
          ...
          vm.registerModel(
            ...
            FakeVasterModel(
                modelName: 'bench-down',
                handler: (_) => throw StateError('API error 503 dead')),
          );
          vm.registerModel(
            const ModelDescriptor(provider: 'bench_up', modelId: 'f'),
            FakeVasterModel(
                modelName: 'bench-up',
                defaultResponseText: 'from the fallback'),
          );
          return vm;
        },
      );
  ...
  static ReliabilityBenchmark _effectsOnce() => ReliabilityBenchmark(
        id: 'effects_once',
        exercises: 'effect ledger (REL-P4): a retried turn replays tool '
        ...
        program: () => _compile([
          const Resilient(
            attempts: 3,
            child: Prompt(Template.text('send the alert'),
...
String renderBenchmarkTable(
    List<({ReliabilityBenchmark benchmark, VariantReport report})> rows) {
  final buffer = StringBuffer()
    ..writeln('| benchmark | exercises | backend | trials | success | '
        'tokens | cost |')
    ..writeln('|---|---|---|---|---|---|---|');
  for (final row in rows) {
    final r = row.report;
    buffer.writeln('| ${row.benchmark.id} '
...
String encodeBenchmarkReports(
        List<({ReliabilityBenchmark benchmark, VariantReport report})>
            rows) =>
