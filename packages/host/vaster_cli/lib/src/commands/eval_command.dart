...
class EvalCommand extends VasterCommand {
  ...
  String get description =>
      'Runs a compiled program N times and reports success rate, score, and '
  ...
  Future<int> execute(CommandContext context) async {
    ...
    final scorer = AllOfScorer([
      const HaltedScorer(),
      for (final needle in results['contains'] as List<String>)
        ContainsScorer(needle),
    ...
    final harness = EvalHarness(
      scorer: scorer,
      trialsPerVariant:
          int.tryParse(results['trials'] as String? ?? '') ?? 3,
    );
