  ...
  return FakeVasterModel(
    handler: (request) async {
      ...
      if (text.contains('Choose exactly one')) {
        return ModelResponse(
          message: ChatMessage.model(jsonEncode({'choice': 'ship', 'rationale': 'quality bar met'})),
          finishReason: FinishReason.stop,
        );
      }
      ...
      if (text.contains('produce the status report')) {
        return ModelResponse(
          message: ChatMessage.model(jsonEncode({'verdict': 'GO', 'confidence': 0.97})),
          finishReason: FinishReason.stop,
        );
      }
...
Pipeline warRoom() => const Pipeline(
  ...
  mounts: [StorageMount(mountPrefix: '/workspace', type: StorageMountType.memory)],
  inputs: {Binding('project'): 'orion', Binding('quality_bar'): 'high'},
  ...
  children: [
    ...
    BudgetScope(
      ...
      child: Knowledge(
        label: 'launch brief',
        text: Template.text('Project orion ships when quality bar is met.'),
        pinned: true,
        child: Sequence([
          // ① Direct prompt through the tool loop.
          Prompt(
            Template.text('Before anything, consult the registry for the launch window.'),
          ...
          CallSubroutine(
            name: 'record_note',
            arguments: {'note_name': 'kickoff', 'note_body': 'war room open for project orion'},
          ),
          ...
          Repeat(
            times: 2,
            children: [
              Prompt(Template.text('brainstorm one launch risk'), output: Binding('risk')),
              WriteFile(path: Template.text('/workspace/risks/risk.txt'), content: Template.text('\${risk}')),
            ],
          ),
          ...
          ParallelTasks(
            entries: [
              ParallelTaskEntry(agentId: 'builder', prompt: 'Build the launch runbook.', output: 'runbook'),
              ParallelTaskEntry(
                agentId: 'reviewer',
          ...
          // ⑥ Sandbox execution.
          Sandbox(
            env: CodeEnvironment(envId: 'calc', language: SandboxLanguage.dart, timeoutMs: 5000),
            child: Execute(code: Template.text('6 * 7'), output: Binding('calc_out')),
          ),
          ...
          TryCatch(
            tryChildren: [ReadFile(path: Template.text('/workspace/missing/nope.txt'))],
            catchChildren: [
              WriteFile(
          ...
          Extract(from: Binding('status_json'), field: 'verdict', output: Binding('verdict')),
                  ...
                  ApprovalGate(
                    ...
                    onApprove: [
                      WriteFile(
                        ...
                        content: Template.text('LAUNCHED \${project}: \${verdict} (calc \${calc_out})'),
                      ),
