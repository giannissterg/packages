...
void main() {
  ...
  test('compiled Decide routes execution through the model-chosen path', () async {
      ...
      handler: (request) {
        ...
        if (text.contains('Choose exactly one')) {
          return ModelResponse(
            message: ChatMessage.model(jsonEncode({'choice': 'escalate', 'rationale': 'severity is high'})),
          );
        }
    ...
    final program = compiler.compile(
      pipeline(result: const Binding('handling'), const [
        Decide(
    ...
    expect(
      prompts.any((p) => p.contains('automatic fix')),
      isFalse,
  ...
  test('DecideLoop iterates until the model decides it is done', () async {
    ...
    final model = FakeVasterModel(
      handler: (request) {
        ...
        if (text.contains('Choose exactly one')) {
          decisions++;
          return ModelResponse(
            message: ChatMessage.model(jsonEncode({'choice': decisions < 3 ? 'continue' : 'done'})),
          );
        }
    ...
    expect(state.status, RuntimeStatus.halted);
    final workSteps = model.recordedRequests.where((r) => r.messages.last.text.contains('next work step'));
    expect(workSteps, hasLength(3), reason: 'continue, continue, done → the body ran exactly 3 times');
    final summaries = model.recordedRequests.where((r) => r.messages.last.text.contains('summarize'));
  ...
  test('DecideLoop maxIterations forces termination on a runaway model', () async {
    final model = FakeVasterModel(
      handler: (request) {
        final text = request.messages.last.text;
        if (text.contains('Choose exactly one')) {
          return ModelResponse(message: ChatMessage.model(jsonEncode({'choice': 'continue'})));
        }
        return ModelResponse(message: ChatMessage.model('step output'));
      },
    );
        ...
        Provider<DecisionPolicy>(
          ...
          children: [
            DecideLoop(
              ...
              continueDescription: 'more work is needed',
              body: [Prompt(Template.text('do the next work step'))],
              exits: [DecisionPath(label: 'done', description: 'task complete')],
            ),
          ],
        ),
      ]),
    ...
    expect(state.status, RuntimeStatus.halted, reason: 'the guard terminates a model that never stops');
    final workSteps = model.recordedRequests.where((r) => r.messages.last.text.contains('next work step'));
