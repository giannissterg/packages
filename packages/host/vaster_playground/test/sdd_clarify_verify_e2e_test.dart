...
void main() {
  ...
  test('Clarify asks until the model is satisfied, folding human answers', () async {
      ...
      handler: (request) {
        ...
        if (text.contains('Choose exactly one')) {
          return ModelResponse(
            message: ChatMessage.model(jsonEncode({'choice': questionsAsked < 2 ? 'ask' : 'ready'})),
          );
        }
        if (text.contains('Ask the single most important')) {
          ...
          return ModelResponse(
            message: ChatMessage.model(
              'Question $questionsAsked: what about '
        ...
        if (text.contains('Update the clarification notes')) {
          ...
          return ModelResponse(message: ChatMessage.model('NOTES(v$questionsAsked) $q -> $a'));
        }
    ...
    expect(
      runtime.pendingHumanRequest!.prompt,
    ...
    state = await runtime.resumeWithHumanResponse(
      HumanInteractionResponse.answer(requestId: 'clarify', answerText: 'Full refunds within 30 days.'),
    );
    ...
    state = await runtime.resumeWithHumanResponse(
      HumanInteractionResponse.answer(requestId: 'clarify', answerText: 'USD and EUR only.'),
    );
  ...
  test('Verify routes pass and fail through the judged verdict', () async {
    Future<(String verdictTaken, String verdict)> runVerify(String sandboxOutput) async {
      final model = FakeVasterModel(
        handler: (request) {
          final text = request.messages.last.text;
          if (text.contains('Choose exactly one')) {
            ...
            return ModelResponse(
              message: ChatMessage.model(jsonEncode({'choice': passed ? 'pass' : 'fail'})),
            );
          }
      ...
      final followUp = model.recordedRequests
          ...
          .where((t) => t.contains('announce the release') || t.contains('open a remediation task'))
          .single;
      final artifact = await vm.fileSystemManager
          .resolveFileSystem('/workspace/verification.md')
          .readText('/workspace/verification.md');
      expect(artifact, contains(sandboxOutput), reason: 'the run output is the verification artifact');
