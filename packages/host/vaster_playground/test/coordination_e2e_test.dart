...
void main() {
  ...
  test('FanOut synthesis prompt receives both interpolated branch outputs', () async {
    ...
    final program = compiler.compile(
      Pipeline(
        ...
        children: const [
          FanOut(
            tasks: [
              ParallelTaskEntry(agentId: 'scout_n', prompt: 'survey the north half', output: 'north'),
              ParallelTaskEntry(agentId: 'scout_s', prompt: 'survey the south half', output: 'south'),
            ],
            synthesize: Task(
    ...
    final synthesisRequest = model.recordedRequests.lastWhere(
      (r) => r.messages.last.text.contains('Merge the surveys'),
    );
  ...
  test('RefineLoop revises until the critic satisfies the decision model', () async {
    var workerRuns = 0;
    final model = FakeVasterModel(
      handler: (request) {
        final text = request.messages.last.text;
        if (text.contains('Choose exactly one')) {
          ...
          return ModelResponse(
            message: ChatMessage.model(jsonEncode({'choice': workerRuns >= 2 ? 'accept' : 'revise'})),
          );
        }
    ...
    final state = await runtime.executeProgram(program);
    expect(state.status, RuntimeStatus.halted);
    expect(workerRuns, equals(2), reason: 'revise once, then accept on round two');

    // Round two's critic saw round two's draft through interpolation.
    final critiques = model.recordedRequests.where(
      (r) => r.messages.last.text.contains('Critique this draft'),
    );
  ...
  test('Router dispatches only the chosen route', () async {
    final model = FakeVasterModel(
      handler: (request) {
        final text = request.messages.last.text;
        if (text.contains('Choose exactly one')) {
          return ModelResponse(message: ChatMessage.model(jsonEncode({'choice': 'billing'})));
        }
    ...
    expect(
      prompts.any((p) => p.contains('Diagnose the crash')),
      isFalse,
  ...
  test('Resilient retries until success and exposes prior errors', () async {
    ...
    final failing = compiler.compile(
      Pipeline(
        ...
        children: const [
          Resilient(
            ...
            attempts: 3,
            onExhausted: [
              Prompt(Template([r'All attempts failed: ', Binding('retry_error')])),
            ],
          ),
        ],
      ),
    ...
    expect(state.status, RuntimeStatus.halted, reason: 'exhaustion flows to onExhausted, not a trap');
    final report = model.recordedRequests.single.messages.last.text;
    expect(report, contains('All attempts failed:'));
    expect(
      report,
      isNot(contains(r'${retry_error}')),
