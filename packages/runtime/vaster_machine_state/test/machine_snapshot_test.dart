...
void main() {
  group('MachineSnapshot', () {
    test('capture folds every registered component; restore dispatches back', () {
      ...
      final snapshot = MachineSnapshot.capture(pc: 7, componentList: [a, b]);

      final a2 = _FakeComponent('a', {});
      final b2 = _FakeComponent('b', {});
      MachineSnapshot.fromJson(jsonDecode(jsonEncode(snapshot.toJson())) as Map<String, dynamic>)
          .restoreInto([a2, b2]);
    ...
    test('restore refuses to drop state for an unknown component key', () {
      ...
      expect(() => snapshot.restoreInto([_FakeComponent('a', {})]), throwsA(isA<StateError>()));
    });
    ...
    test('ErrorHandlerFrame round-trips', () {
      ...
      final restored =
          ErrorHandlerFrame.fromJson(jsonDecode(jsonEncode(frame.toJson())) as Map<String, dynamic>);
