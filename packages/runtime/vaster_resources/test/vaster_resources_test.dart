...
void main() {
  test('consumption returns the running balance; applyQuota returns the '
    ...
    expect(tracker.consumeTokens(50), 150, reason: 'the balance composes without a second getter call');
    ...
    expect(displaced, same(ResourceQuota.unlimited), reason: 'the quota it replaced is data, not a mystery');
  ...
  group('ResourceQuota & ResourceTracker', () {
    test('tracks token usage and throws when quota exceeded', () {
      ...
      expect(() => tracker.consumeTokens(60), throwsA(isA<QuotaExceededException>()));
    });

    test('tracks tool call executions and throws when limit exceeded', () {
      final tracker = ResourceTracker(quota: const ResourceQuota(maxToolCallsPerTask: 2));

      tracker.recordToolCall();
      tracker.recordToolCall();
      expect(tracker.toolCallCount, equals(2));

      expect(() => tracker.recordToolCall(), throwsA(isA<QuotaExceededException>()));
    });

    test('checks deadline expiration', () async {
      final tracker = ResourceTracker(quota: const ResourceQuota(timeDeadline: Duration(milliseconds: 50)));

      await Future.delayed(const Duration(milliseconds: 70));
      expect(() => tracker.checkDeadline(), throwsA(isA<QuotaExceededException>()));
    });
  });
}
