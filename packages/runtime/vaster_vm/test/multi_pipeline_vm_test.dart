...
void main() {
  group('Multi-Pipeline VM Scheduler Composition Tests', () {
    ...
    test('submits and time-slices execution across multiple AST programs concurrently', () async {
      ...
      final job1 = vm.submitProgram(
        program1,
        policy: ExecutionPolicy.unlimited,
        budget: vm.rootBudget.createChildBudget(),
        priority: TaskPriority.high,
      );
      final job2 = vm.submitProgram(
        program2,
        policy: ExecutionPolicy.unlimited,
        budget: vm.rootBudget.createChildBudget(),
    ...
    test('quanta run through the scheduler state machine, not around it', () async {
      ...
      expect(
        firstState.status,
        equals(RuntimeStatus.running),
        reason:
            'a 2-instruction quantum of a 4-instruction program '
      ...
      expect(vm.scheduler.taskQueue.isEmpty, isTrue, reason: 'no orphaned quanta after the job halts');
    });
    ...
    test('a job whose budget is exhausted lands as timedOut, not stranded', () async {
      ...
      expect(job.isDone, isTrue, reason: 'budget expiry at dispatch must terminate the job');
    });

    test('with cores > 1, one job\'s model I/O overlaps another job\'s call', () async {
      ...
      final slowModel = _SlowModel(delay: const Duration(milliseconds: 60), onSpan: spans.add);
      final smpVm = await VasterVMEngine.bootstrap(config: VMConfig(defaultModel: slowModel, cores: 2));

      final jobA = smpVm.submitProgram(
        const VasterProgram(
          programName: 'io_a',
          instructions: [
            PromptOp(promptText: 'A turn'),
            HaltOp(),
          ],
        ),
        policy: ExecutionPolicy.unlimited,
      ...
      expect(
        spans[1].start.isBefore(spans[0].end),
        isTrue,
    ...
    test('a HITL pause parks the job without re-enqueueing quanta', () async {
      ...
      expect(vm.scheduler.taskQueue.isEmpty, isTrue, reason: 'a parked job must not spin in the run queue');
    });
  });
}
...
class _SlowModel implements VasterModel {
  ...
  @override
  Stream<ModelResponseChunk> generateStream(ModelRequest request) => _inner.generateStream(request);
}
