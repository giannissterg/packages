...
void main() {
  group('Runtime register interpolation', () {
    ...
    test('PromptOp text interpolates a prior output register', () async {
      ...
      expect(
        model.recordedRequests.single.messages.last.text,
        equals('Summarize findings about billing errors.'),
      );
    });

    test('WriteFileOp interpolates content and path; ReadFileOp reads it back', () async {
      const program = VasterProgram(
        programName: 'interp_vfs',
        instructions: [
    ...
    test('unresolved reference stays verbatim and emits RuntimeWarningEvent', () async {
      final warnings = <RuntimeWarningEvent>[];
      vm.eventBus.on<RuntimeWarningEvent>().listen(warnings.add);

      const program = VasterProgram(
        programName: 'interp_warn',
        instructions: [
      ...
      expect(model.recordedRequests.single.messages.last.text, equals(r'about ${never_written}'));
    ...
    test('MountFsOp with diskPath mounts a real local filesystem', () async {
      final tempDir = await Directory.systemTemp.createTemp('vaster_interp_mount_');
      final program = VasterProgram(
        programName: 'disk_mount',
        instructions: [
      ...
      expect(onDisk.existsSync(), isTrue, reason: 'a declared diskPath must produce a real file on disk');
    ...
    test('RegisterSandboxOp timeoutMs bounds sandbox execution', () async {
      vm.registerSandbox(
        IsolateCodeSandbox(
          ...
          defaultPolicy: const SandboxSecurityPolicy(maxTimeout: Duration(milliseconds: 50)),
