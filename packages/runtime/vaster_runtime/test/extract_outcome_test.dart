...
void main() {
  group('RegisterFile.jsonExtract outcomes', () {
    ...
    test('a present key extracts and reports ExtractOk with the value', () {
      ...
      final outcome = registers.jsonExtract(sourceVar: 'src', jsonKey: 'verdict', targetVar: 'out');
    ...
    test('an unset source reports ExtractSourceMissing, target unset', () {
      final outcome = registers.jsonExtract(sourceVar: 'ghost', jsonKey: 'k', targetVar: 'out');
      expect(outcome, isA<ExtractSourceMissing>().having((o) => o.sourceVar, 'src', 'ghost'));
    ...
    test('unparseable JSON reports ExtractParseFailure, target unset', () {
      ...
      final outcome = registers.jsonExtract(sourceVar: 'src', jsonKey: 'k', targetVar: 'out');
    ...
    test('a non-object JSON value reports ExtractParseFailure', () {
      ...
      expect(
        outcome,
        isA<ExtractParseFailure>().having((o) => o.detail, 'detail', contains('not a JSON object')),
      );
    });
    ...
    test('a missing key reports ExtractKeyMissing with the available keys', () {
      ...
      expect(
        ...
        isA<ExtractKeyMissing>()
            ...
            .having((o) => o.availableKeys, 'availableKeys', containsAll(['Verdict', 'confidence'])),
    ...
    test('an already-decoded Map source extracts without re-parsing', () {
      ...
      final outcome = registers.jsonExtract(sourceVar: 'src', jsonKey: 'nested', targetVar: 'out');
