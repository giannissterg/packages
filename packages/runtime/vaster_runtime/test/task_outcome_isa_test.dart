...
void main() {
  Future<(VasterVirtualMachine, VasterRuntime)> boot(
      FakeVasterModel model) async {
  ...
  FakeVasterModel alwaysFailing() => FakeVasterModel(
      handler: (_) => throw StateError('HTTP 500 from the backend'));

  test('an unhandled agent failure TRAPS — never a silent empty register',
      () async {
    ...
    expect(state.registers['out'], isNull,
        reason: 'no phantom empty-string output');
  ...
  test('a handler catches the failure; the outcome kind is observable',
      () async {
    final (vm, runtime) = await boot(alwaysFailing());
    final state = await runtime.executeProgram(const VasterProgram(
      ...
      instructions: [
        ...
        DispatchAgentTaskOp(
            agentId: 'worker', taskPrompt: 'try it', outputVar: 'out'), // 1
        PopErrorHandlerOp(), // 2
        JumpOp(targetPc: 5), // 3
    ...
    expect(state.registers['recovered'], 'yes',
        reason: 'TryCatch around a Task finally works');
    expect(state.registers['err'], contains('model-failure'),
        reason: 'the handler sees the classified failure');
    expect(state.registers[taskOutcomeRegister('out')], 'model-failure');
    await vm.shutdown();
  });

  test('parallel dispatch writes every outcome, keeps successes, then raises',
      () async {
    var calls = 0;
    ...
    final state = await runtime.executeProgram(VasterProgram(
      ...
      instructions: [
        ...
        const DispatchParallelTasksOp(dispatches: [
          ParallelTaskDispatch(
              agentId: 'a1', taskPrompt: 'good work', outputVar: 'r1'),
          ParallelTaskDispatch(
              agentId: 'a2', taskPrompt: 'bad work', outputVar: 'r2'),
        ]),
    ...
    expect(state.registers['r1'], 'fine',
        reason: 'the succeeding branch keeps its output');
    expect(state.registers[taskOutcomeRegister('r2')], 'model-failure');
    expect(state.registers['r2'], isNull);
    expect(state.registers['err'], contains('Agent task failed'),
        reason: 'the batch failure reached the handler AFTER all '
            'outcomes were written');
    await vm.shutdown();
  });

  test('cancellation and quota classify distinctly through the same path',
      () async {
