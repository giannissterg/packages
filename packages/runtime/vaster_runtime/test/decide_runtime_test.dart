...
void main() {
  group('DecideOp — model-steered branching', () {
    ...
    //   4: took_right = true
    //   5: halt
    VasterProgram program({String? outputVar = 'choice', String? defaultLabel}) => VasterProgram(
      programName: 'decide_demo',
      instructions: [
        DecideOp(
    ...
    test('model choice steers control flow; label and rationale land in registers', () async {
      final model = FakeVasterModel(
        handler: (request) {
          ...
          expect((schema['properties'] as Map)['choice']['enum'], equals(['left', 'right']));
          return ModelResponse(
            message: ChatMessage.model(jsonEncode({'choice': 'right', 'rationale': 'the right is clear'})),
          );
        },
      );
    ...
    test('markdown-fenced JSON answers parse', () async {
      final model = FakeVasterModel(
        handler: (_) {
          return ModelResponse(message: ChatMessage.model('```json\n{"choice": "left"}\n```'));
        },
      );
      final (vm, runtime) = await boot(model);
    ...
    test('unresolvable answer takes the default branch and flags the event', () async {
    ...
    test('a failed decision is catchable by a program-level error handler', () async {
      final model = FakeVasterModel(
        handler: (_) {
      ...
      final state = await runtime.executeProgram(
        const VasterProgram(
          ...
          instructions: [
            ...
            DecideOp(
              prompt: 'choose',
              branches: [DecisionBranch(label: 'only', description: 'the one', targetPc: 2)],
            ),
            HaltOp(),
      ...
      expect(
        ...
        contains('nonsense'),
        reason:
            'DecisionUnresolved carries the raw model answer into the '
    ...
    test('empty branch list traps immediately', () async {
      ...
      expect(model.recordedRequests, isEmpty, reason: 'no model call is made for a malformed decide');
      await vm.shutdown();
    });
    ...
    test('decision consumes budget and publishes DecisionMadeEvent', () async {
      ...
      final model = FakeVasterModel(
        handler: (_) {
          return ModelResponse(message: ChatMessage.model(jsonEncode({'choice': 'left'})));
        },
      );
      final vm = await VasterVMEngine.bootstrap(config: VMConfig(defaultModel: model));
      vm.eventBus.on<DecisionMadeEvent>().listen(events.add);
    ...
    test('session-routed decision appends to conversational history', () async {
      ...
      expect(
        history.first.text,
        contains('- left: go left'),
        reason: 'the branch menu is presented to the model',
      );
      await vm.shutdown();
    });

    test('a response carrying functionCalls still decides from its text', () async {
      final model = FakeVasterModel(
        handler: (_) {
          return ModelResponse(
            message: ChatMessage(
      ...
      final state = await runtime.executeProgram(program());
      expect(state.registers['took_right'], isTrue, reason: 'decisions are atomic — no tool loop runs');
      expect(model.recordedRequests, hasLength(1), reason: 'exactly one model call, no tool continuation');
      await vm.shutdown();
    });
  });
}
