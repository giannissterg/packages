...
void main() {
  ModelTape fixtureTape() => const ReplayEnvelopeCodec()
      .decodeString(
        File('../../host/vaster_playground/test/fixtures/sdd_fidelity.replay.json').readAsStringSync(),
      )
      .tape;
  ...
  group('TapeCalibrationFitter on the committed fixture', () {
    ...
    setUpAll(
      () => fit = const TapeCalibrationFitter().fit(
        fixtureTape(),
    ...
    test('re-derived profile matches the committed constants', () {
      expect(
        fit.calibration.charsPerToken,
        closeTo(CalibrationCatalog.geminiFlash.charsPerToken, 1e-9),
        reason:
            'the builtin catalog must be refittable from the fixture '
            '— committed numbers cannot silently rot',
      );
      expect(fit.calibration.sampleCount, CalibrationCatalog.geminiFlash.sampleCount);
    });
    ...
    test('exclusion is loud: the implausible sample is reported', () {
      expect(
        ...
        1,
        reason:
            'one fixture call has more tokens than characters — '
    ...
    test('error bounds hold — and beat the flat heuristic', () {
      expect(fit.meanAbsErrorFraction, lessThan(0.20), reason: 'fixture mean error stays under 20%');
      ...
      for (final entry in fixtureTape().entries) {
        ...
        heuristicErr += (TokenEstimate.forText(response.text) - tokens).abs() / tokens;
        calibratedErr += (calibrated.forText(response.text) - tokens).abs() / tokens;
        n++;
      }
      expect(n, fit.calibration.sampleCount);
      expect(
        calibratedErr / n,
        lessThan(heuristicErr / n),
        reason:
            'the entire point: fitted knowledge estimates the '
  ...
  group('fitter refuses to lie', () {
    ModelTapeEntry entry(String text, int tokens) => ModelTapeEntry(
      ...
      responseJson: ModelResponse(
        ...
        usage: UsageMetadata(candidatesTokenCount: tokens, promptTokenCount: 1, source: UsageSource.measured),
      ).toJson(),
    );
    ...
    test('too few plausible samples is an error, not a guess', () {
      ...
      expect(
        () => const TapeCalibrationFitter().fit(tape, backendId: 'b', provenance: 'test'),
        throwsA(isA<StateError>().having((e) => e.message, 'message', contains('refusing to fit'))),
      );
    });
    ...
    test('estimated-source usage never contributes to a fit', () {
          ...
          for (var i = 0; i < 5; i++)
            ModelTapeEntry(
              ...
              responseJson: ModelResponse(
                ...
                usage: const UsageMetadata(candidatesTokenCount: 6, source: UsageSource.estimated),
              ).toJson(),
            ),
        ],
      );
      expect(
        ...
        throwsStateError,
        reason:
            'fitting estimates to estimates would launder the '
  ...
  group('CalibratedTokenEstimator composes with the seam', () {
    test('implements the same TokenEstimator contract as the heuristic', () {
      ...
      final TokenEstimator calibrated = CalibratedTokenEstimator(CalibrationCatalog.geminiFlash);
      const text = 'Some representative stretch of English prose.';
      expect(
        heuristic.forText(text),
        TokenEstimate.forText(text),
        reason: 'the default delegates — identical by construction',
      );
      expect(
        calibrated.forText(text),
        isNot(heuristic.forText(text)),
        reason: 'fitted knowledge changes the number',
      );
      expect(
        calibrated.forExchange(prompt: text, output: text).source,
  ...
  test('claude-cli overhead factor carries its weakness openly', () {
    ...
    expect(
      profile.callOverheadFactor,
      closeTo(0.1157 / 0.0518, 0.01),
  ...
  group('CalibrationCatalog is an instance, composable', () {
    test('builtin lookup and miss', () {
      expect(CalibrationCatalog.builtin.forBackend('gemini-2.0-flash'), same(CalibrationCatalog.geminiFlash));
    ...
    test('a custom catalog composes without touching the builtin', () {
      const mine = CalibrationCatalog([
        EstimateCalibration(backendId: 'my-local', charsPerToken: 3.2, sampleCount: 10, provenance: 'test'),
      ]);
      expect(mine.forBackend('my-local')!.charsPerToken, 3.2);
      expect(
        mine.forBackend('gemini-2.0-flash'),
        isNull,
  ...
  group('prompt-side fitting (tape v2)', () {
    ModelTape v2Tape() => const ReplayEnvelopeCodec()
        .decodeString(
          File('../../host/vaster_playground/test/fixtures/story_lines_v2.replay.json').readAsStringSync(),
        )
        .tape;

    test('fits from full recorded requests with measured prompt usage', () {
      final fit = const TapeCalibrationFitter().fitPromptSide(
        v2Tape(),
      ...
      expect(
        fit.meanAbsErrorFraction,
        lessThan(0.35),
        reason:
            'prompt-side estimates hold within a stated bound on '
    ...
    test('v1 tapes cannot contribute — the limitation tape v2 removes', () {
      expect(
        () => const TapeCalibrationFitter().fitPromptSide(
          fixtureTape(),
          backendId: 'gemini-2.0-flash',
          provenance: 'v1',
        ),
        throwsA(
          isA<StateError>().having((e) => e.message, 'message', contains('v1 entries cannot contribute')),
        ),
      );
    });
  });
