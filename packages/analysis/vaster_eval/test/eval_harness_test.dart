...
void main() {
  ...
  EvalVariant variant(String label, VasterModel Function() model) => EvalVariant(
    label: label,
    program: program,
    vmFactory: () async => VasterVMEngine.bootstrap(config: VMConfig(defaultModel: model())),
    dispose: (vm) => (vm as VasterVMEngine).shutdown(),
  );

  test('a deterministic passing variant scores 100% with metered totals', () async {
    final harness = EvalHarness(
      scorer: const AllOfScorer([HaltedScorer(), ContainsScorer('GO')]),
      trialsPerVariant: 3,
    );
    final report = await harness.run([
      variant('always_go', () => FakeVasterModel(defaultResponseText: 'verdict: GO')),
    ]);
    ...
    expect(v.totalTokens, greaterThan(0), reason: 'trials carry real metered numbers');
  ...
  test('a FLAKY model yields an honest fractional success rate', () async {
    ...
    var call = 0;
    VasterModel flaky() => FakeVasterModel(
      handler: (request) async {
        call++;
        return ModelResponse(
          message: ChatMessage.model(call.isOdd ? 'verdict: GO' : 'verdict: NO'),
          finishReason: FinishReason.stop,
        );
      },
    );

    final harness = EvalHarness(scorer: const ContainsScorer('GO'), trialsPerVariant: 4);
    final report = await harness.run([variant('flaky', flaky)]);

    final v = report.variants.single;
    expect(v.successRate, 0.5, reason: '2 of 4 trials pass — the number no single run can give you');
    expect(v.passed, 2);
    final failing = v.trials.where((t) => !t.score.passed);
    expect(
      failing.every((t) => t.score.detail!.contains('GO')),
      isTrue,
  ...
  test('variants are comparable side by side in one report', () async {
    final harness = EvalHarness(scorer: const ContainsScorer('GO'), trialsPerVariant: 2);
    final report = await harness.run([
    ...
    expect((json['variants'] as List), hasLength(2), reason: 'the report is a serializable artifact');
  });

  test('a trapping program fails the HaltedScorer with its trap detail', () async {
    ...
    final harness = EvalHarness(scorer: const HaltedScorer(), trialsPerVariant: 1);
    final report = await harness.run([
      EvalVariant(
        label: 'trapper',
        program: trapping,
        vmFactory: () async => VasterVMEngine.bootstrap(config: VMConfig(defaultModel: FakeVasterModel())),
        dispose: (vm) => (vm as VasterVMEngine).shutdown(),
      ),
    ]);
