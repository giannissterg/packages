...
void main() {
  ProgramChecker checker({ExecutionPolicy? policy, String? model}) =>
      ProgramChecker(pricingCatalog: PricingCatalog.builtin, policy: policy, modelName: model);
  ...
  group('binding dominance (definite assignment)', () {
    ...
    test('a read of a never-written register is an error', () {
      ...
      expect(
        ...
        isA<ReadNeverWritten>().having((f) => f.register, 'register', 'ghost').having((f) => f.pc, 'pc', 0),
      );
    ...
    test('a write on only ONE branch is a possibly-unset warning — the flat '
      ...
      expect(
        report.findings.single,
        isA<PossiblyUnsetRead>().having((f) => f.register, 'register', 'maybe').having((f) => f.pc, 'pc', 4),
      );
      ...
      final report = checker().check(
        VasterProgram(
          ...
          instructions: [
            ...
            const DecideOp(
              prompt: 'choose',
              branches: [DecisionBranch(label: 'a', description: 'a', targetPc: 3)],
              outputVar: 'pick',
            ), // 2
            const WriteFileOp(vfsPath: '/mem/o.txt', content: r'${pick} because ${pick_rationale}'), // 3
            const HaltOp(), // 4
          ],
        ),
      );
      expect(report.findings, isEmpty, reason: 'runtime-written siblings are part of the ABI');
    });
  ...
  group('cost bounds', () {
    test('a bounded loop multiplies its calls; the bound is finite', () {
      ...
      final report = checker(model: 'claude-sonnet-5').check(
        const VasterProgram(
          programName: 'bounded',
          instructions: [
            SetRegisterOp(registerName: 'i', value: 0), // 0
            CompareRegisterOp(leftVar: 'i', operator: 'lt', rightValue: 4, targetVar: 'ok'),
    ...
    test('agent dispatch counts its tool-loop ceiling', () {
      ...
      expect(report.costBound.maxModelCalls, 3, reason: 'one dispatch × the descriptor tool-loop ceiling');
    });
    ...
    test('with no rated model, the bound derives the most expensive '
      ...
      const program = VasterProgram(
        ...
        instructions: [
          SelectModelOp(
            descriptor: ModelDescriptor(provider: 'google_ai', modelId: 'gemini-2.5-flash'),
            fallbacks: [
      ...
      expect(mostExpensiveSelectableModel(program, PricingCatalog.builtin), 'gemini-2.5-pro');

      final derived = checker().check(program).costBound;
      final explicit = checker(model: 'gemini-2.5-pro').check(program).costBound;
      expect(
        derived.maxCostUsd,
        isNotNull,
        reason:
            'a program with priceable selections is never rated '
    ...
    test('agent-declared chains are selectable too — the rated model may '
        ...
        instructions: [
          CreateAgentOp(
            descriptor: AgentDescriptor(
              ...
              role: 'r',
              systemInstruction: 's',
              modelDescriptor: ModelDescriptor(provider: 'google_ai', modelId: 'gemini-2.0-flash'),
              modelFallbacks: [
      ...
      expect(mostExpensiveSelectableModel(program, PricingCatalog.builtin), 'claude-opus-5');
    });
    ...
    test('a program selecting no priceable model keeps the honest '
      ...
      expect(mostExpensiveSelectableModel(program, PricingCatalog.builtin), isNull);
  ...
  group('policy proofs', () {
    ...
    test('a statically denied write is a PROVEN violation (error)', () {
      final report = checker(policy: readOnly).check(
        const VasterProgram(
      ...
      expect(
        ...
        isA<PolicyViolationProven>().having((f) => f.resource, 'resource', '/mem/x.txt'),
      );
      expect(report.hasErrors, isTrue);
    });
    ...
    test('an unreachable violation is NOT reported — proofs are about '
      ...
      expect(report.findings.whereType<UnreachableInstruction>(), hasLength(1));
    });
    ...
    test('a clean read-only program is fully proven', () {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'every reachable action allowed, no dynamic resources — '
            'a proof',
      );
    });
  });
  ...
  group('cost bound composes the estimation seam', () {
    ...
    test('defaults are byte-identical to the heuristic era', () {
      ...
      expect(plain.maxTokens, TokenEstimate.forText('a prompt of some length for the bound') + 1024);
    });
    ...
    test('the harness overhead factor scales the whole bound', () {
      ...
      expect(
        agentic.maxCostUsd,
        greaterThan(api.maxCostUsd! * 2.2),
        reason:
            'the prove-it lesson, priced in: CLI-agentic calls cost '
...
final class _FixedRatioEstimator implements TokenEstimator {
  ...
  @override
  int forMessages(Iterable<ChatMessage> messages) => messages.fold(0, (s, m) => s + forText(m.text));
  @override
