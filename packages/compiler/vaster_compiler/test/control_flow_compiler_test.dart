...
void main() {
  ...
  Pipeline pipeline(List<VasterNode> children) => Pipeline(name: 'control_flow_test', children: children);
  ...
  group('Repeat lowering', () {
    test('emits counter init, compare, body, increment, and back-jump', () {
      ...
      final init = instructions.whereType<SetRegisterOp>().firstWhere((op) => op.registerName == 'i');
      expect(init.value, 0);
      ...
      final hasBackJump = instructions.indexed.any((e) => e.$2 is JumpOp && (e.$2 as JumpOp).targetPc < e.$1);
  ...
  group('While lowering', () {
    test('emits maxIterations guard compare and user-condition jumpIf', () {
      ...
      final conditionJump = instructions.whereType<JumpIfOp>().where((op) => op.conditionVar == 'go');
  ...
  group('Subroutine lowering', () {
    test('body is emitted after halt; call targets it; return closes it', () {
      ...
      expect(call.targetPc, greaterThan(haltPc), reason: 'subroutine bodies live after the main halt');
    ...
    test('call to an undefined subroutine is a compile error', () {
      final result = compiler.compileWithDiagnostics(pipeline(const [CallSubroutine(name: 'ghost')]));
      expect(result.hasErrors, isTrue);
      expect(result.errors.single.code, 'undefined_subroutine');
      expect(result.errors.single.message, contains('ghost'));

      expect(() => compiler.compile(pipeline(const [CallSubroutine(name: 'ghost')])), throwsStateError);
    });
  });
  ...
  group('Analyzer integration', () {
    test('control-flow programs produce no error diagnostics', () {
      ...
      expect(result.hasErrors, isFalse, reason: result.errors.map((e) => '$e').join('\n'));
    });

    test('optimizer keeps subroutine bodies alive (call-referenced labels)', () {
      const optimizing = BasicWorkflowCompiler(options: CompilerOptions(optimize: true));
      final program = optimizing.compile(
        pipeline(const [
      ...
      expect(program.instructions.whereType<ReturnSubroutineOp>(), hasLength(1));
    });
  });
  ...
  group('Decide lowering', () {
    test('branch targets land on each path; paths rejoin after the node', () {
      ...
      expect(instructions.whereType<PromptOp>().where((p) => p.promptText == 'after join'), hasLength(1));
    });
    ...
    test('the chosen label lands in the declared result binding', () {
      final program = compiler.compile(
        const Pipeline(
          ...
          children: [
            Decide(
              prompt: Template.text('pick'),
              output: Binding('choice'),
              paths: [
    ...
    test('optimize:true preserves every decide path block', () {
      ...
      final prompts = program.instructions.whereType<PromptOp>().map((p) => p.promptText);
      expect(
        prompts,
        containsAll(['path a body', 'path b body']),
        reason:
            'branch blocks are only reachable through the decide — '
    ...
    test('analyzer rejects duplicate labels and unknown defaults', () {
      ...
      final duplicate = analyzer.analyze(
        const VasterProgram(
          programName: 'dup',
          instructions: [
            DecideOp(
      ...
      final unknownDefault = analyzer.analyze(
        const VasterProgram(
          programName: 'unk',
          instructions: [
            DecideOp(
              prompt: 'p',
              defaultLabel: 'ghost',
              branches: [DecisionBranch(label: 'x', description: 'a', targetPc: 1)],
            ),
            HaltOp(),
          ],
        ),
  ...
  group('DecideLoop lowering', () {
    test('continue branch is a back-edge; exhaustion routes to an exit', () {
      ...
      final instructions = program.instructions;
      final decideIndex = instructions.indexWhere((inst) => inst is DecideOp);
      final decide = instructions[decideIndex] as DecideOp;

      expect(decide.branches.first.label, 'continue');
      expect(decide.branches.first.targetPc, lessThan(decideIndex), reason: 'continue is the loop back-edge');
      ...
      final compare = instructions.whereType<CompareRegisterOp>().single;
      expect(compare.rightValue, 3);
      final forcedExit = instructions.whereType<JumpOp>().any((j) => j.targetPc == doneBranch.targetPc);
    ...
    test('onContinue lowers on the continue edge, before the back-jump', () {
      ...
      final continueTarget = decide.branches.firstWhere((b) => b.label == 'revise').targetPc;
      expect(continueTarget, greaterThan(decideIndex), reason: 'continue routes forward through onContinue');
      expect((instructions[continueTarget] as PromptOp).promptText, equals('apply the fixes'));

      // ...and onContinue jumps back into the loop (a backward jump).
      final backJump = instructions.skip(continueTarget).whereType<JumpOp>().first;
      expect(backJump.targetPc, lessThan(decideIndex), reason: 'after revising, control re-enters the loop');
    });

    test('Review(revise:) closes the loop: decide-first with regeneration', () {
      AgentRole role(String id) => AgentRole(roleId: id, name: id, title: id, instruction: 'You: $id');
      final program = compiler.compile(
        Pipeline(
          name: 'review_loop',
      ...
      final decide = instructions.whereType<DecideOp>().single;
      expect(decide.branches.map((b) => b.label), containsAll(['revise', 'approve']));
    ...
    test('Provider<DecisionPolicy> supplies maxIterations; node field wins', () {
        ...
        Provider<DecisionPolicy>(
          ...
          children: [
            DecideLoop(
              ...
              exits: const [DecisionPath(label: 'done', description: 'complete')],
            ),
          ],
        ),
      ]);
      ...
      expect(fromPolicy.instructions.whereType<CompareRegisterOp>().single.rightValue, 5);

      final fromNode = compiler.compile(loopWith(nodeMax: 2));
      expect(fromNode.instructions.whereType<CompareRegisterOp>().single.rightValue, 2);
    });
  });
}
