...
void main() {
  group('BudgetShare', () {
    test('floor and ceiling combine tokens and fractions', () {
      const share = BudgetShare(
          minTokens: 500, minFraction: 0.1, maxTokens: 5000, maxFraction: 0.5);
  ...
  group('ContextClassTable', () {
    test('standard table is valid and band-ordered', () {
      ...
      final bands =
          ContextClassTable.standard.inBandOrder.map((c) => c.name).toList();
      expect(
          bands,
          equals(
              ['system', 'tools', 'knowledge', 'general', 'history', 'scratch']));
    });
    ...
    test('withOverrides replaces and extends; JSON round-trips', () {
      ...
      expect(restored.resolve('scratch').lifetime,
          equals(ContextLifetime.ephemeral));
      expect(restored.resolve('history').eviction,
          equals(EvictionPolicy.dropOldest));
    });

    test('totalReservedFor sums hard floors', () {
      final table = ContextClassTable.standard.withOverrides([
        const ContextClass(
            name: 'knowledge', band: 20, share: BudgetShare(minTokens: 3000)),
        const ContextClass(
            name: 'system', band: 0, share: BudgetShare(minFraction: 0.1)),
      ]);
      expect(table.totalReservedFor(10000), equals(3000 + 1000));
    });

    test('validate flags mismatched keys and missing default class', () {
      const bad = ContextClassTable(classes: {
        'a': ContextClass(name: 'b', band: 0),
  ...
  group('ContextRegion policy inheritance', () {
    ...
    test('null fields inherit from the class; explicit fields override', () {
      final inheriting = ContextRegion.text(
          id: 'r1', label: 'r1', role: Role.user, text: 'hello');
      expect(inheriting.effectivePriority(highClass),
          equals(ContextPriority.high));
      expect(inheriting.effectiveLifetime(highClass),
          equals(ContextLifetime.persistent));
      expect(inheriting.effectiveCompressibility(highClass),
      ...
      expect(overriding.effectivePriority(highClass),
          equals(ContextPriority.low));
      expect(overriding.effectiveLifetime(highClass),
          equals(ContextLifetime.ephemeral));
      expect(overriding.effectiveCompressibility(highClass),
    ...
    test('legacy OrDefault views reproduce pre-class defaults', () {
      final region = ContextRegion.text(
          id: 'r', label: 'r', role: Role.user, text: 'x');
      ...
      expect(region.compressibilityOrDefault,
          equals(ContextCompressibility.none));
    });
    ...
    test('source-sync merge preserves heap-side classId and overrides', () {
      ...
      heap.addRegion(ContextRegion.text(
          id: 'doc', label: 'doc', role: Role.user, text: 'v1'));
      // Operator assigns a class + pin on the heap side.
      heap.updateRegion(
          'doc', (r) => r.copyWith(classId: 'knowledge', isPinned: true));
