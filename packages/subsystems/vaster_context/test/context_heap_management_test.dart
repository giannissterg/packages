...
ContextRegion _region(
  String id,
  String text, {
  bool pinned = false,
  ContextPriority priority = ContextPriority.medium,
}) =>
    ContextRegion.text(id: id, label: id, role: Role.user, text: text, isPinned: pinned, priority: priority);
...
void main() {
  group('ContextHeap management primitives', () {
    test('updateRegion mutates in place without reordering', () {
      ...
      expect(ok, isTrue);
      expect(heap.regions.map((r) => r.id), equals(['a', 'b', 'c']), reason: 'order preserved');
    ...
    test('upsertFromSource preserves heap-side policy and compressed shadows', () {
      ...
      heap.upsertFromSource(_region('doc', 'original document content'), fingerprintOf: regionFingerprintOf);
      ...
      heap.upsertFromSource(_region('doc', 'brand new document content'), fingerprintOf: regionFingerprintOf);
  ...
  group('New model surface', () {
    test('compressibility parse and copyWith clearCompression', () {
      expect(ContextCompressibility.parse('summarize'), equals(ContextCompressibility.summarize));
      expect(ContextCompressibility.parse('bogus'), equals(ContextCompressibility.none));
      ...
      final region = _region('r', 'text').copyWith(
        compression: CompressionInfo(compressorId: 't', tokensBefore: 10, sourceFingerprint: 'fp'),
      );
