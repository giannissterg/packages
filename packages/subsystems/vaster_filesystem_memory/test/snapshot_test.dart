...
void main() {
  group('MemoryVasterFileSystem export/import', () {
    test('text and binary files round-trip through base64', () async {
      ...
      await fs.writeBytes('/bin/blob', Uint8List.fromList([0, 1, 255, 128, 7]));

      final restored = MemoryVasterFileSystem()..importFilesBase64(fs.exportFilesBase64());

      expect(await restored.readText('/notes/a.txt'), 'héllo — unicode ✓');
      expect(await restored.readBytes('/bin/blob'), equals([0, 1, 255, 128, 7]));
    });
    ...
    test('import replaces same-path files and keeps others', () async {
      ...
      fs.importFilesBase64((MemoryVasterFileSystem()..importFilesBase64(const {})).exportFilesBase64());
