...
void main() {
  ContextRegion bigRegion(String id, {String? classId, int messages = 12}) =>
      ContextRegion(
        id: id,
        label: id,
        messages: [
          for (var i = 0; i < messages; i++)
            ChatMessage.user('message $i of $id — ${'x' * 80}'),
        ],
  ...
  test('never-eviction classes are immune to compaction', () async {
    ...
    final report = await compactor.compact(
      ...
      regions: [
        bigRegion('sys', classId: 'system').copyWith(
            compressibility: ContextCompressibility.truncate, isPinned: false),
      ],
      targetTokens: 100,
      includePinned: true,
