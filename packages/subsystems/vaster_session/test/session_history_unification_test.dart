...
void main() {
  group('SessionHistorySource projection', () {
    test('chunks + tail with stable ids, priorities, and order', () {
      final history = [
        for (var i = 0; i < 19; i++) i.isEven ? ChatMessage.user('turn $i') : ChatMessage.model('turn $i'),
      ];
      final source = SessionHistorySource(sessionId: 's1', historyProvider: () => history);
    ...
    test('empty history projects nothing', () {
      final source = SessionHistorySource(sessionId: 's1', historyProvider: () => []);
      expect(source.getRegions(), isEmpty);
    });
  });

  group('BasicModelSession — unified history', () {
    test('request contains each history message exactly once, in order', () async {
    ...
    test('ambient heap regions render before history', () async {
      ...
      final session = BasicModelSession(sessionId: 'chat', model: fakeModel, contextManager: manager);
      manager.addRegion(
        ContextRegion.text(
          id: 'ambient_doc',
          label: 'doc',
      ...
      final texts = fakeModel.recordedRequests.last.messages.map((m) => m.text).toList();
      expect(
        texts.indexWhere((t) => t.contains('AMBIENT DOCUMENT')),
        lessThan(texts.indexOf('question about the doc')),
      );
    });

    test('compressed history chunk survives subsequent sends (shadowing)', () async {
      final fakeModel = FakeVasterModel();
      final manager = BasicContextManager(compressors: [const TruncatingCompressor(keepTailMessages: 1)]);
      final session = BasicModelSession(sessionId: 'long', model: fakeModel, contextManager: manager);
      ...
      await manager.compact(targetTokens: 20, regionId: 'session:long:history:0');
      ...
      expect(
        manager.getRegion('session:long:history:0')!.isCompressed,
        isTrue,
    ...
    test('clearHistory removes projected regions', () async {
      final fakeModel = FakeVasterModel();
      final manager = BasicContextManager();
      final session = BasicModelSession(sessionId: 'wipe', model: fakeModel, contextManager: manager);
      await session.send(ChatMessage.user('hello'));
      expect(manager.regions.where((r) => r.id.startsWith('session:wipe:history')), isNotEmpty);

      session.clearHistory();
      expect(session.history, isEmpty);
      expect(manager.regions.where((r) => r.id.startsWith('session:wipe:history')), isEmpty);
    });

    test('ephemeral-lifetime scratch regions are pruned at turn end', () async {
      final fakeModel = FakeVasterModel();
      final manager = BasicContextManager();
      final session = BasicModelSession(sessionId: 'scratch', model: fakeModel, contextManager: manager);
      manager.addRegion(
        ContextRegion.text(
          id: 'scratchpad',
          label: 'scratch',
      ...
      expect(manager.getRegion('scratchpad'), isNull, reason: 'turn boundary expires ephemeral regions');
    });
  });
}
