...
class CowFileSnapshot {
  ...
  const CowFileSnapshot({required this.pages, required this.dirtyPaths, required this.deletedPaths});
  ...
  factory CowFileSnapshot.fork(CowFileSnapshot parent) {
    return CowFileSnapshot(pages: Map.of(parent.pages), dirtyPaths: {}, deletedPaths: {});
  }

  /// Returns an empty initial snapshot.
  factory CowFileSnapshot.empty() {
    return const CowFileSnapshot(pages: {}, dirtyPaths: {}, deletedPaths: {});
  }
  ...
  factory CowFileSnapshot.fromJson(Map<String, dynamic> json) {
    ...
    return CowFileSnapshot(
      pages: rawPages.map((path, encoded) => MapEntry(path as String, base64Decode(encoded as String))),
      dirtyPaths: ((json['dirtyPaths'] as List?) ?? const []).map((e) => e as String).toSet(),
      deletedPaths: ((json['deletedPaths'] as List?) ?? const []).map((e) => e as String).toSet(),
    );
  }
}
