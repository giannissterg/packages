...
void main() {
  ...
  Uint8List buildGolden() {
    final bytes = Uint8List(
      const KvStateImageCodec().layoutSize(
  ...
  group('round trip', () {
    test('empty state, empty tokens, empty fingerprint', () {
      final bytes = Uint8List(
        const KvStateImageCodec().layoutSize(contentFingerprint: '', tokenCount: 0, stateSize: 0),
      );
    ...
    test('multibyte UTF-8 fingerprint survives', () {
      ...
      final bytes = Uint8List(
        const KvStateImageCodec().layoutSize(contentFingerprint: fp, tokenCount: 2, stateSize: 3),
      );
      final image = const KvStateImageCodec().initialize(
        bytes,
        tokenIds: const [5, 6],
    ...
    test('state section alignment holds across fingerprint/token sizes', () {
      for (final fpLen in [0, 1, 3, 4, 7, 8, 31, 64]) {
        for (final n in [0, 1, 2, 3, 5, 129]) {
          final fp = 'x' * fpLen;
          final bytes = Uint8List(
            const KvStateImageCodec().layoutSize(contentFingerprint: fp, tokenCount: n, stateSize: 1),
          );
          final image = const KvStateImageCodec().initialize(
            bytes,
          ...
          expect(image.stateOffset % 8, 0, reason: 'state must be 8-aligned (fp=$fpLen, n=$n)');
          expect(
            image.tokenIds,
            List<int>.generate(n, (i) => i - 2),
    ...
    test('views are zero-copy: writing through them mutates the buffer', () {
      ...
      expect(
        const KvStateImageCodec().parse(bytes).stateBytes[0],
        0x11,
        reason: 'stateBytes is a view over the same memory',
      );
      image.tokenIds[0] = 42;
      expect(
        const KvStateImageCodec().parse(bytes).tokenIds[0],
        42,
  ...
  group('validation (spec §Consuming, steps 1–3)', () {
    test('bad magic names both values and suspects pre-format frames', () {
      ...
      expect(
        ...
        throwsA(
          isA<KvStateImageFormatException>().having((e) => e.message, 'message', contains('pre-format')),
        ),
      );
    });
    ...
    test('unknown version rejected', () {
      ...
      expect(() => const KvStateImageCodec().parse(bytes), throwsA(isA<KvStateImageFormatException>()));
    });
    ...
    test('unknown flags rejected — v1 forward-compat discipline', () {
      ...
      expect(
        () => const KvStateImageCodec().parse(bytes),
        throwsA(isA<KvStateImageFormatException>().having((e) => e.message, 'message', contains('flags'))),
      );
    });
    ...
    test('every truncation point is caught', () {
      ...
      for (final cut in [0, 4, 35, 36, 45, 47, 48, 60, 63, 64, 68]) {
        expect(
          () => const KvStateImageCodec().parse(Uint8List.sublistView(golden, 0, cut)),
          throwsA(isA<KvStateImageFormatException>()),
          reason: 'a $cut-byte buffer must be rejected as truncated',
        );
      }
      // The exact length parses.
      expect(const KvStateImageCodec().parse(Uint8List.sublistView(golden, 0, 69)), isA<KvStateImage>());
    });
    ...
    test('non-zero padding is corruption', () {
      ...
      expect(
        ...
        throwsA(
          isA<KvStateImageFormatException>().having(
            (e) => e.message,
            'message',
            contains('fingerprint padding'),
          ),
        ),
      );
      ...
      expect(
        ...
        throwsA(
          isA<KvStateImageFormatException>().having((e) => e.message, 'message', contains('token padding')),
        ),
      );
    });
    ...
    test('misaligned container base is a typed container bug', () {
      ...
      expect(
        () => const KvStateImageCodec().parse(misaligned),
        throwsA(isA<KvStateImageAlignmentException>()),
      );
      expect(
        () => const KvStateImageCodec().initialize(
          misaligned,
          tokenIds: const [1],
  ...
  group('prefixDivergence (spec §Consuming, step 5)', () {
    ...
    test('exact prefix permits reuse (-1)', () {
      ...
      expect(
        image.prefixDivergence([1, 2, 32000, 9, 9]),
        -1,
    ...
    test('an empty prefix is a prefix of anything', () {
      final bytes = Uint8List(
        const KvStateImageCodec().layoutSize(contentFingerprint: 'e', tokenCount: 0, stateSize: 0),
      );
      final empty = const KvStateImageCodec().initialize(
        bytes,
        tokenIds: const [],
