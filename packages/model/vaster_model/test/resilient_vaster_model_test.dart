...
void main() {
  ModelRequest request() => ModelRequest(messages: [ChatMessage.user('hello')]);

  /// A fake that fails the first [failures] calls with [error], then succeeds.
  FakeVasterModel flaky(int failures, Object Function() error, {String name = 'flaky', String reply = 'ok'}) {
    var calls = 0;
    return FakeVasterModel(
      ...
      handler: (req) {
        ...
        return ModelResponse(
          ...
          usage: const UsageMetadata(promptTokenCount: 1, candidatesTokenCount: 1),
        );
      },
    );
  }
  ...
  group('defaultIsTransient', () {
    test('classifies retryable statuses and timeouts as transient', () {
      ...
      expect(defaultIsTransient(StateError('Claude API error 429 rate')), isTrue);
      expect(defaultIsTransient(StateError('Claude API error 503 busy')), isTrue);
      expect(defaultIsTransient(StateError('Claude API error 529 overloaded')), isTrue);
    });

    test('classifies client errors and auth as permanent', () {
      expect(defaultIsTransient(StateError('Claude API error 400 bad')), isFalse);
      expect(defaultIsTransient(StateError('Claude API error 401 auth')), isFalse);
  ...
  group('ResilientVasterModel.generate', () {
    ...
    test('permanent errors do not retry the same model', () async {
      final primary = flaky(99, () => StateError('API error 400 bad request'), name: 'primary');
      final model = ResilientVasterModel(
        primary: primary,
      ...
      await expectLater(model.generate(request()), throwsStateError);
      expect(primary.recordedRequests, hasLength(1), reason: 'a 400 will fail identically on every retry');
    });

    test('falls back to the next model after primary exhausts', () async {
      final primary = flaky(99, () => StateError('API error 500'), name: 'primary');
      final backup = flaky(0, () => StateError('unused'), name: 'backup', reply: 'from backup');
    ...
    test('permanent error still advances to a fallback provider', () async {
      final primary = flaky(99, () => StateError('API error 401 bad key'), name: 'primary');
      final backup = flaky(0, () => StateError(''), name: 'backup', reply: 'rescued');
      final model = ResilientVasterModel(primary: primary, fallbacks: [backup], sleep: noSleep);
      final response = await model.generate(request());
    ...
    test('cancellation is a decision, not a model failure — never advances', () async {
      final primary = FakeVasterModel(
        modelName: 'primary',
        handler: (req) => throw const CancelledException('caller cancelled'),
      );
      final backup = flaky(0, () => StateError('unused'), name: 'backup', reply: 'must never serve');
      final model = ResilientVasterModel(
        primary: primary,
        fallbacks: [backup],
        retryPolicy: const RetryPolicy(maxAttempts: 3),
      ...
      await expectLater(model.generate(request()), throwsA(isA<CancelledException>()));
      expect(primary.recordedRequests, hasLength(1), reason: 'cancellation must not be retried');
      expect(backup.recordedRequests, isEmpty, reason: 'cancellation must not advance the chain');
    });
    ...
    test('per-attempt timeout converts a hang into a retry', () async {
      ...
      final model = ResilientVasterModel(
        primary: hanging,
        retryPolicy: const RetryPolicy(maxAttempts: 2, attemptTimeout: Duration(milliseconds: 50)),
        sleep: noSleep,
      );
      final response = await model.generate(request());
