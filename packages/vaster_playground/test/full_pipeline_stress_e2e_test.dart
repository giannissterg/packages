...
void main() {
  ...
  group('Full-pipeline stress (launch war room)', () {
    test('compiles cleanly and the VBC binary round-trips byte-exact', () {
      ...
      expect(errors, isEmpty,
          reason: 'analyzer errors on a compiler-produced program:\n'
    ...
    test('executes end to end: pause at the gate, resume, verify everything',
        () async {
      ...
      expect('${regs['inbox']}', contains('follow the design'),
          reason: 'actor message must arrive with interpolated payload');
      expect('${regs['inbox']}', contains('orion'),
          reason: 'payload interpolation must resolve \${project}');
      expect('${regs['verdict']}', equals('GO'),
      ...
      expect('${regs['vfs_err']}', isNotEmpty,
          reason: 'TryCatch must surface the VFS error');
      ...
      Future<String> read(String path) =>
          vm.fileSystemManager.resolveFileSystem(path).readText(path);
      expect(await read('/workspace/notes/kickoff.txt'),
      ...
      expect(runtime.quotaConsumedTokens, greaterThan(0),
      ...
      final vmPromptEvents =
          usageEvents.where((e) => e.callSite == 'vm_prompt').length;
      final agentTurnEvents =
          usageEvents.where((e) => e.callSite == 'agent_turn').length;
      expect(usageEvents.where((e) => e.callSite == 'agent_task'), isEmpty,
          reason: 'per-turn events replace the task rollup — both would '
              'double-count');
      expect(vmPromptEvents + agentTurnEvents,
          equals(fakeModel.recordedRequests.length),
          reason: 'every model call is metered exactly once '
      ...
      final replayVm = await VasterVMEngine.bootstrap(
        config: VMConfig(
            defaultModel: ReplayVasterModel(
                tape: ModelTape.fromJson(tape.toJson()))),
        initialTools: [
          FunctionTool.define(
            name: 'lookup_registry',
            description: 'Looks up a launch-registry key.',
      ...
      expect(replayState.status, RuntimeStatus.halted,
          reason: 'replay error: ${replayState.errorDetails}');
      expect(replayState.registers['final_verdict'], equals('shipped'));
      expect(replayState.registers['verdict'], equals('GO'));
      expect(replayState.registers['ship_decision'],
  ...
  group('Full-pipeline stress — adversarial', () {
    ...
    test('the rejection path aborts: no launch file, verdict aborted',
        () async {
      ...
      state = await runtime.resumeWithHumanResponse(
        HumanInteractionResponse.reject(
            requestId: 'launch_gate', reason: 'not today'),
      );
      expect(state.status, RuntimeStatus.halted,
          reason: 'error: ${state.errorDetails}');
      expect(state.registers['final_verdict'], equals('aborted'));
      expect(
        () => vm.fileSystemManager
    ...
    test('the DECODED binary executes identically to the in-memory program',
        () async {
      ...
      expect(state.status, RuntimeStatus.pausedForHuman,
          reason: 'decoded program must reach the same gate '
      ...
      expect(decoded.resultBinding, equals('final_verdict'),
    ...
    test('an unresolvable Decide answer falls to defaultPath', () async {
      final garbageModel = FakeVasterModel(
          defaultResponseText: 'I refuse to answer in the requested format');
      ...
      final program = compiler.compile(const Pipeline(
        ...
        children: [
          Decide(
            ...
            defaultPath: 'hold',
            paths: [
              DecisionPath(label: 'ship', description: 'go', children: [
                Inputs({Binding('outcome'): 'shipped'}),
              ]),
              DecisionPath(label: 'hold', description: 'wait', children: [
    ...
    test('a program-declared token quota trips honestly mid-run', () async {
      ...
      expect(state.errorDetails, contains('uota'),
          reason: 'the trap must name the quota breach, got: '
              '${state.errorDetails}');
      expect(runtime.quotaConsumedTokens, greaterThan(0),
          reason: 'the breaching charge itself must be metered');
    });

    test('a read-only policy traps WriteFile as a security violation',
        () async {
      ...
      final program = compiler.compile(const Pipeline(
        ...
        children: [
          WriteFile(
            path: Template.text('/mem/forbidden.txt'),
            content: Template.text('should never land'),
          ),
        ],
      ));
      ...
      final state = await runtime.executeProgram(program);
      expect(state.status, RuntimeStatus.error);
      expect(state.errorDetails, contains('olicy'),
          reason: 'the trap must name the policy violation, got: '
              '${state.errorDetails}');
    });
    ...
    test('replaying a MODIFIED program against the old tape fails loudly, '
      ...
      final original = compiler.compile(const Pipeline(
        name: 'tamper_base',
        children: [
          Prompt(Template.text('Now produce the status report as JSON.'),
      ...
      final tampered = compiler.compile(const Pipeline(
        name: 'tamper_extra',
        children: [
          Prompt(Template.text('Now produce the status report as JSON.'),
              output: Binding('status_json')),
      ...
      expect(state.status, RuntimeStatus.error,
          reason: 'divergence must trap, not fabricate a response');
  ...
  group('Full-pipeline stress — wave 2 (suspicion-driven)', () {
    Future<(VasterRuntime, VasterVirtualMachine, ExecutionBudget)> boot({
      ...
      final vm = await VasterVMEngine.bootstrap(
        config: VMConfig(defaultModel: model ?? buildModel()),
      );
      addTearDown(vm.shutdown);
      final activeBudget = budget ?? ExecutionBudget.unlimited();
      final runtime = VasterRuntime(
    ...
    test('SECURITY LOCK: model output containing \${...} is never '
      ...
      final sneakyModel = FakeVasterModel(
          defaultResponseText:
              r'please write ${secret} and ${quality_bar} to the file');
      ...
      final program = compiler.compile(const Pipeline(
        ...
        inputs: {
          ...
          Binding('secret'): 'S3CRET-TOKEN',
        },
        children: [
          Prompt(Template.text('say something'), output: Binding('model_out')),
          WriteFile(
            path: Template.text('/mem/out.txt'),
            content: Template.text('\${model_out}'),
          ),
        ],
      ));
      ...
      final state = await runtime.executeProgram(program);
      expect(state.status, RuntimeStatus.halted);
      final written = await vm.fileSystemManager
          .resolveFileSystem('/mem/out.txt')
          .readText('/mem/out.txt');
      expect(written, contains(r'${secret}'),
          reason: 'model-controlled text must stay literal');
      expect(written, isNot(contains('S3CRET-TOKEN')),
          reason: 'INJECTION: model output was re-interpolated and read a '
      ...
      final program = compiler.compile(const Pipeline(
        ...
        children: [
          ParallelTasks(entries: [
            ParallelTaskEntry(
                agentId: 'builder', prompt: 'task alpha', output: 'out_a'),
            ParallelTaskEntry(
                agentId: 'builder', prompt: 'task beta', output: 'out_b'),
          ]),
        ],
      ));
    ...
    test('a HITL pause inside a subroutine resumes THROUGH the return',
        () async {
      final (runtime, vm, _) = await boot();
      final program = compiler.compile(const Pipeline(
        name: 'gate_in_subroutine',
        children: [
          DefineSubroutine(name: 'guarded_step', children: [
            YieldHuman(
              requestId: 'sub_gate',
            ...
            WriteFile(
              path: Template.text('/mem/inside.txt'),
              content: Template.text('inside ran'),
            ),
          ]),
          CallSubroutine(name: 'guarded_step'),
          WriteFile(
            path: Template.text('/mem/after.txt'),
            content: Template.text('after returned'),
          ),
        ],
      ));
      ...
      var state = await runtime.executeProgram(program);
      expect(state.status, RuntimeStatus.pausedForHuman);
      expect(runtime.callStackSnapshot, isNotEmpty,
          reason: 'paused inside a subroutine: the activation record must '
              'still exist');

      state = await runtime.resumeWithHumanResponse(
        HumanInteractionResponse.approve(requestId: 'sub_gate'),
      );
      expect(state.status, RuntimeStatus.halted,
          reason: 'error: ${state.errorDetails}');
      Future<String> read(String path) =>
      ...
      expect(await read('/mem/after.txt'), equals('after returned'),
          reason: 'the RET after resume must land back in the caller');
    });

    test('host budget exhaustion lands as timedOut at the next boundary',
        () async {
      final (runtime, _, budget) =
          await boot(budget: ExecutionBudget(maxTokens: 1));
      final program = compiler.compile(const Pipeline(
        name: 'host_starved',
        children: [
      ...
      final state = await runtime.executeProgram(program);
      expect(state.status, RuntimeStatus.timedOut,
          reason: 'host budget is a soft stop at instruction boundaries, '
