  ...
  test('two scoped SDD cycles compose without collisions', () async {
    ...
    final pipeline = Pipeline(
      ...
      children: [
        ...
        BindingScope(
            namespace: 'billing', child: cycle('Build the billing flow.')),
      ],
    );
    ...
    for (final name in [
      'auth_spec', 'auth_plan', 'auth_review', 'auth_review_verdict',
      'billing_spec', 'billing_plan', 'billing_review',
      'billing_review_verdict',
    ]) {
      expect(state.registers.containsKey(name), isTrue,
          reason: 'missing binding $name');
    }
    expect(state.registers['auth_spec'],
        isNot(equals(state.registers['billing_spec'])));
    ...
    expect(
        await fs.readText('/workspace/billing_spec.md'), contains('billing'));
